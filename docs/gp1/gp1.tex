...
\begin{singlespace}
    ...
    \begin{tabbing}
        ...
        Calendar \> A list of events and dates within a year that are \\
        \> important to an organization or to the people involved in \\
        \> a particular activity. \cite{def:calendar} \\[0.5em]
...
\begin{itemize}
    ...
    \item Integration with iOS's native calendar system through EventKit framework.
    \item Secure storage of calendar account credentials.
...
\begin{itemize}
    ...
    \item Integration with EventKit might require careful handling of permissions and access rights.
    \item Local notifications need to be managed efficiently to avoid overwhelming the user.
    \item Learning new technologies for iOS development might require more time than anticipated.
    ...
    \item We may face challenges to test the app due to lack of users for testing our app
\end{itemize}
...
The functionality of Jadwal can be best understood through its various use cases, which demonstrate how users interact with the system. Each use case details specific interactions and flows that make up the core functionality of Jadwal.
...
\textbf{Figure~\ref{fig:use-case-diagram}} shows the complete use case diagram for Jadwal's system. The diagram illustrates how:
\begin{itemize}
    \item Users interact with calendars through iOS's EventKit framework
    \item The system extracts events from WhatsApp messages
    \item The backend securely stores calendar credentials
    \item Local conflict detection happens on the device
    \item Prayer times are scheduled and managed
\end{itemize}
...
At its core, Jadwal helps users manage their calendars effectively through iOS's native calendar system. These use cases show how users can create new calendars, connect existing ones through EventKit, and view all their calendars in one integrated interface.
...
Managing events and resolving scheduling conflicts are daily challenges for users. These use cases demonstrate how Jadwal leverages EventKit to handle event creation and provide smart conflict detection directly on the device.
...
\subsection{Prayer Time Management}
Prayer time scheduling is a unique feature of Jadwal that helps users maintain their daily religious observances while managing their schedule effectively.
...
The foundation of Jadwal's functionality lies in its focused and efficient database architecture. This section presents both the Entity-Relationship model and Relational Schema that support the application's core features: user authentication and secure calendar credential storage. The database design ensures secure credential management while leveraging iOS's native calendar infrastructure for all calendar operations.
...
\begin{enumerate}
    ...
    \item \textbf{Calendar Credentials}
          \begin{itemize}
              \item \texttt{caldav\_account}: Stores encrypted calendar account credentials
              \item Securely manages usernames and passwords for calendar services
              \item Links customers to their calendar providers
          \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Authentication Tables}
          \begin{itemize}
              \item \texttt{customer} (\underline{id}, email, created\_at, updated\_at)
              \item \texttt{auth\_google} (\underline{id}, customer\_id, sid, created\_at, updated\_at)
              \item \texttt{magic\_link} (\underline{id}, customer\_id, hashed\_token, expires\_at, created\_at, updated\_at)
          \end{itemize}

    \item \textbf{Calendar Account Table}
          \begin{itemize}
              \item \texttt{caldav\_account} (\underline{id}, customer\_id, email, username, password, created\_at, updated\_at)
          \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Foreign Keys}: Attributes ending in "\_id" represent foreign key relationships
    \item \textbf{Audit Fields}: All tables include created\_at and updated\_at timestamps
    \item \textbf{Security Features}:
          \begin{itemize}
              \item Passwords are encrypted using pgcrypto extension
              \item UUID used for identifiers to prevent enumeration
              \item Timestamps are timezone-aware
          \end{itemize}
\end{itemize}
...
Jadwal leverages iOS's native calendar system through EventKit, providing seamless integration with all calendar services supported by iOS. This allows users to effortlessly manage their calendars from different providers like Apple Calendar and Google Calendar, all through iOS's built-in calendar infrastructure.

The feature that makes Jadwal stand out is its state-of-the-art LLM WhatsApp event extraction feature, which enhances the application by automatically detecting and adding events from informal communications. This solves a critical problem where important events discussed in chat messages often get missed or lead to conflicts.

Prayer time scheduling highlights Jadwal's unique approach to time management, recognizing the importance of daily religious observances. By automatically blocking and managing prayer times, Jadwal helps users maintain their spiritual commitments while efficiently organizing their other activities.

The intelligent event management system offers features like local conflict detection and resolution through EventKit, ensuring real-time synchronization and reliable calendar operations. By utilizing iOS's native calendar capabilities alongside our smart conflict resolution system, Jadwal provides a comprehensive solution for managing overlapping commitments.

A key advantage of our architecture is that users can continue using their preferred calendar apps alongside Jadwal, as all changes are synchronized through iOS's calendar system. This creates a seamless experience where users can view and manage their events through any iOS calendar app while benefiting from Jadwal's enhanced features like WhatsApp integration and prayer time management.
...
In conclusion, Jadwal stands out from all the existing solutions and provides an innovative calendar application that bridges the gaps and helps the user to manage their day more efficiently. With its comprehensive features including WhatsApp integration, prayer time scheduling, intelligent conflict resolution, and seamless calendar synchronization through EventKit, Jadwal is poised to revolutionize the way individuals manage their time in today's fast-paced world.
