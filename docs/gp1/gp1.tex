...
Clockwise's website tries to touch the user's misery by saying "Your calendar doesn't have to suck" as shown in Figure \ref{fig:clockwise-landing}.A Clockwise is a web-based application consider one of the smartest calendar assistant that optimizes schedules and manages team coordination \cite{clockwise}.Analyzes data across multiple calendars at once and considers preferences across individuals, teams, and companies simultaneously Clockwise's approach to intelligent time blocking and meeting optimization provides valuable insights for Jadwal's automated scheduling features \cite{clockwise}.

Understands which meetings are flexible, evaluates likelihood of resolving conflicting events[1] \cite{clockwise}.Clockwise provided as a paid verison.It can integrate with  different external sources such as Google and Microsoft and other management application which help the user to set their preferences and optimize their daily routine.Clockwise block event consider as either free or busy.which make easy and simple.However it dosnot support the prioritizing time block which consider limition to some users.Moreover, it lacks adding context around what the user is doing .
...
Jadwal platform, such as linking the calendar to WhatsApp for automatic event addition and integrating prayer times, which adds a unique level of customization and day organization.Motion gives you everything you need to organize complicated projects, and then uses AI to turn hundreds of tasks into an optimized plan, so that you can just focus on executing instead of worrying about planning and deadlines\cite{motion}.It is web-based application.Motion one of the main application for small business projects.It help user to create task and identify  deadline and its priority and break down the project into manageable steps.Motion support integration with majority of the calenders (Gmail, Outlook, and iCloud).That helps the user to manage all the events, block and task in one place.Set your working hours. Motion will calculate your weekly tasks, meetings, and total working hours. It then builds a custom schedule to get it done — without working late into the night\cite{motion}.
...
While these features enhance productivity and optimize time management, Reclaim.ai does not include features like WhatsApp integration for automatically adding events or prayer time scheduling, both of which are unique to the Jadwal platform. These features provide users with cultural and practical benefits that further enhance their daily organization.Reclaim is an AI calendar assistant that uses flexible time blocking to find the best time for your meetings, tasks, habits, and breaks around your existing schedule. Reclaim was built for people who have calendars that are slammed with of their calendar being taken up by meetings and/or task work \cite{reclaim}.Block time of Reclaim AI is more specific that contains for each block task ,routines,meeting and breaks,which help to increase the availability of the meeting and give to the schedulers or the team insights into your priorites and less to get interrupt.Reclaim Ai integrate with  Google Calendar, Gmail, Google Tasks, Jira, ClickUp, Todoist, Linear, Asana, as well as Zoom and Google Meet. An advanced Slack integration allows you to manage your agenda, create Tasks from Slack messages, check off todos, start and stop events, join meetings, share Scheduling Links, and sync your Slack status to your calendar \cite{reclaim}.It is web-based application and provided with montly fees.Include group video call that consider the availability of the group.Inaddition,It uses AI to schedule appointments based on your preferences and old events.Reclaim AI limitations that we still use another calender for daily routine.

\subsection{Calendi}

Calendi.ai claims to offer features focused on speeding up scheduling with AI-powered assistance. It claims to allow users to add tasks via text or voice input and automates meetings scheduling, including sending links and calendar invites. Calendi claims to integrate with platforms like Google Calendar, Apple Calendar, and Outlook, making it compatible across multiple systems. It claims to provide intelligent assistance by analyzing habits and tasks, suggesting improvements, and incorporating external factors like weather or transit schedules. However, it does not offer unique features like WhatsApp integration or prayer time scheduling, which are distinct to Jadwal. Also the website as of writing this part, on 26 Nov 2024, is broken and not working.Calendi is the fastest and easiest way to schedule anything — from tasks to team collaboration, saving you valuable time and energ \cite{calendi}.Web-based application that is provided free for the users so they can manage their time accurately.Calendi analyzes your tasks and habit to suggest for possible enhancement,It acts as your personal assistance ai and Send reminders to ensure you stay on track. You are able to fix the reminding time or automate it \cite{calendi}. Easily add tasks with text and voice. Calendi has advance algorithm fasten the scheduling time and provide more convenience managing the calendar. with the option to connect many calendars \cite{calendi}.As it support integration with other calender such as Outlook ,Gmail and etc.Calendi uses AI for automated scheduling solution.
